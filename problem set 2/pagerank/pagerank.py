...
def transition_model(corpus, page, damping_factor):
    ...
    if len(corpus[page]) < 1:
        ...
        rr = (1 - damping_factor) / b
        even_factor = damping_factor / c

        for key in corpus.keys():
            if key not in corpus[page]:
                a[key] = rr
            else:
                a[key] = even_factor + rr
        ...
        for page in corpus:
            ...
            for linking_page in corpus:
                if page in corpus[linking_page]:
                    temp += (b[linking_page] / len(corpus[linking_page]))
                if len(corpus[linking_page]) == 0:
                    temp += (b[linking_page]) / len(corpus)
            temp *= damping_factor
            temp += (1 - damping_factor) / a
            c[page] = temp

        di = max([abs(c[x] - b[x]) for x in b])
        if di < 0.001:
            break
        else:
            b = c.copy()
